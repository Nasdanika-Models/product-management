 ...
 */
public class ProductManagementGroovyDslTests {

    @Test
    public void testNasdanikaProductManagementGroovyResource() throws IOException {
		CapabilityLoader capabilityLoader = new CapabilityLoader();
		ProgressMonitor progressMonitor = new PrintStreamProgressMonitor();
		Requirement<ResourceSetRequirement, ResourceSet> requirement = ServiceCapabilityFactory.createRequirement(ResourceSet.class);
		ResourceSet resourceSet = capabilityLoader.loadOne(requirement, progressMonitor);
		
		File markdownFile = new File("src/test/resources/nasdanika.pm.groovy").getCanonicalFile();
		Resource markdownResource = resourceSet.getResource(URI.createFileURI(markdownFile.getAbsolutePath()), true);
		
		File ecoreFile = new File("target/nasdanika-product-model-groovy.xml").getCanonicalFile();
