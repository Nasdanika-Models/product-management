package org.nasdanika.models.productmanagement.ecore;
...
@EPackageNodeProcessorFactory(nsURI = ProductManagementPackage.eNS_URI)
public class EcoreGenProductManagementProcessorsFactory {

	private Context context;
	
	public EcoreGenProductManagementProcessorsFactory(Context context) {
		this.context = context;
	}
		
	@EPackageNodeProcessorFactory(
			label = "Product Management Model",
			description = "Model of the Product Management domain",
			actionPrototype = """
                    app-action:
                      content:
                        content-markdown:
                          source:
                            content-resource:
                              location: product-management.md
					"""
	)
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					page.setName("Product Management");
					...
					Files.writeString(new File("target/product-management.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
