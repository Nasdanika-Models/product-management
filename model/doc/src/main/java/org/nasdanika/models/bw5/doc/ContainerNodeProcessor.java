...
public class ContainerNodeProcessor<T extends Container> extends NamedElementNodeProcessor<T> {

	public ContainerNodeProcessor(
	...
	protected boolean isCallOutgoingReferenceLabelsSuppliers(EReference eReference) {
		if (Bw5Package.Literals.CONTAINER__TRANSITIONS.equals(eReference) ) {
			return false;
		}
		return Bw5Package.Literals.CONTAINER__ACTIVITIES.equals(eReference)
				|| Bw5Package.Literals.CONTAINER__GROUPS.equals(eReference)
				|| super.isCallOutgoingReferenceLabelsSuppliers(eReference);
	}
